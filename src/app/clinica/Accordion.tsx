          ...
          return (
            <div key={index}>
              <button
                ...
                className={`border cursor-pointer w-full flex justify-between items-center lg:px-6 px-4 lg:py-3 py-2 bg-[#353a2c] text-[#e5e6e1] font-light tracking-[1px] uppercase lg:text-[13px] text-[12px] ${
              ...
              >
                {item.titulo}
                <span
                  className={`transition-transform duration-500 ease-in-out will-change-transform font-black text-[14px] lg:text-[21px] ${
              ...
              {/* Wrapper em grid para animar height com grid-rows */}
              <div
                className={`grid transition-[grid-template-rows] duration-[1200ms] ease-in-out bg-[#abb48d] ${
                  isOpen ? "grid-rows-[1fr] py-0" : "grid-rows-[0fr] py-0"
                }`}
              >
                {/* A célula do grid, que “clipa” o conteúdo */}
                <div className="overflow-hidden">
                  <div className="px-0 py-4 grid lg:grid-cols-2 grid-cols-1 gap-x-6 lg:gap-y-4 gap-y-3 text-[#e5e6e1] border-r-1 border-l-1 lg:text-[13px] text-[12px]">
