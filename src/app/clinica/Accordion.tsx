        ...
        {procedimentos.map((item, index) => (
          <div key={index} className="">
            <button
              ...
              className={`w-full flex justify-between items-center lg:px-6 px-4 lg:py-3 py-2 bg-[#353a2c] text-[#e5e6e1] border-t-1 border-[#e5e6e1] font-light tracking-[1px] uppercase lg:text-[13px] text-[0.66rem] ${index === procedimentos.length - 1 ? 'rounded-b-[0.5rem]' : ''}`}
              type="button"
            >
              {item.titulo}
              <span
                className={`transition-transform duration-500 ease-in-out font-black text-[10px] lg:text-[21px] ${openIndex === index ? "rotate-180" : "rotate-0"}`}
              >
                {openIndex === index ? "−" : "+"}
              </span>
            </button>
            ...
            >
              {openIndex === index && (
                <div className="px-0 lg:py-4 py-2 grid grid-cols-2 gap-x-6 lg:gap-y-4 gap-y-[2px] text-[#e5e6e1] border-2 lg:text-[13px] text-[7.35px]">
                  {item.conteudo.map((linha, i) => (
                    linha.includes("Exclusivas") ? (
                      <p key={i} className="font-semibold lg:py-2 bg-[#353a2c] text-[#e5e6e1]  col-span-2 px-2 lg:px-6">{linha}</p>
                    ) : (
